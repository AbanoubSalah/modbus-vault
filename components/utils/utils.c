 ...
 *
 * @brief Implementation of the Utility
 *
 * @details
...
#ifdef CONFIG_ENABLE_MODBUS_CRC16_LUT
uint16_t calculate_modbus_crc16 (const uint8_t *data_ptr, uint32_t length)
{
    ...
    uint16_t current_crc = 0xFFFFU;
    for (uint32_t byte_idx = 0; byte_idx < length; ++byte_idx)
    {
        uint8_t lutIndex = (data_ptr[byte_idx]) ^ current_crc;
        current_crc >>= 8;
        current_crc ^= lut[lutIndex];
    }
...
uint16_t calculate_modbus_crc16 (const uint8_t *data_ptr, uint32_t length)
{
    ...
    for (uint32_t byte_idx = 0; byte_idx < length; ++byte_idx)
    {
        crc ^= (uint16_t) data_ptr[byte_idx];
